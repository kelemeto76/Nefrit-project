// ================================================== //
// # GameServer 1.00.90 WzAG.dll					# //
// # RMST Storm & Tornado Projects 2010-2011		# //
// ================================================== //

#ifndef Logger_H
#define Logger_H

extern char Message[1024];

class Logger
{
public:	  

	enum sColor
	{
		c_Red,		// Крассный
		c_Green,	// Зелёный
		c_Blue,		// Голубой
		c_Cyan,		// Бирюзовый
		c_Yellow,	// Жёлтый
		c_Magenta,	// Фиолетовый
		c_Grey,		// Серый 
		с_Aqua,
		c_Gold,
		c_MistyRose,
		c_Coral,
		c_Indigo,
		c_Navy
	};

	enum sLogType
	{
		t_NULL,
		t_Error,
		t_Default,
		t_GM,
		t_POST,
		t_DROP,
		t_BAN,
		t_COMMANDS,
		t_GPOST,
		t_PCPOINT,
		t_IPBLOCK,
		t_EXCHANGE,
		t_RESET,
		t_Duel,
		t_MARRY,
		t_VIP
	};
									
	Logger();
	~Logger();		 
	void LoggerInit();	  		 
	void CheckProcent(char* message); 	
	void SaveFile(char *logString,char *Message);
	void ConsoleCommands(char* InputCommand);
	int StdIn(char* Buffer);
	void ConsoleOutPut(int WOL, sColor Color, sLogType Type, const char* Format, ...);
	HANDLE Handle(BOOL Input);
	void CreateLog(sLogType Type,const char* Format, ...);	   
	
	bool GetArraySubChar(char* InPutMsg,int &CountSubChar,char ParamChar);

	bool Logger::CheckConsoleCommand(char* CommandHeader,char* CommandSyntax,char** ParamArray,bool Digits[],int CountSubString,int CheckPlayer);

	bool IsDigitsChar(char* inmsg);
	long int CharToInt(char* inmsg);

	char **Param;
};
extern Logger cLog;		

#endif Logger_H