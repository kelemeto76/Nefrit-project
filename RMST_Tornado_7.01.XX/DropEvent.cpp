...
{
	if ((rand() % 100 + 1) <= cexDropEvent.dropCountBC.countRate)
	{
		for (int i=0; i < cexDropEvent.dropCountBC.countItems; i++)
		{
			cexDropEvent.handlerBC(aIndex, MapNumber, x, y, type, level, dur , Op1, Op2, Op3, LootIndex, NewOption, SetOption);
		}
	}
	else {
		cexDropEvent.handlerBC(aIndex, MapNumber, x, y, type, level, dur , Op1, Op2, Op3, LootIndex, NewOption, SetOption);
...
{
	if ((rand() % 100 + 1) <= cexDropEvent.dropCountIT.countRate)
	{
		for (int i=0; i < cexDropEvent.dropCountIT.countItems; i++)
		{
			cexDropEvent.handlerIT(aIndex, MapNumber, x, y, type, level, dur , Op1, Op2, Op3, LootIndex, NewOption, SetOption);
		}
	}
	else {
		cexDropEvent.handlerIT(aIndex, MapNumber, x, y, type, level, dur , Op1, Op2, Op3, LootIndex, NewOption, SetOption);
...
{
	if ((rand() % 100 + 1) <= cexDropEvent.dropCountWW.countRate)
	{
		for (int i=0; i < cexDropEvent.dropCountWW.countItems; i++)
		{
			cexDropEvent.handlerWW(aIndex, MapNumber, x, y, type, level, dur , Op1, Op2, Op3, LootIndex, NewOption, SetOption);
		}
	}
	else {
		cexDropEvent.handlerWW(aIndex, MapNumber, x, y, type, level, dur , Op1, Op2, Op3, LootIndex, NewOption, SetOption);
...
{
	if ((rand() % 100 + 1) <= cexDropEvent.dropCountCC.countRate)
	{
		for (int i=0; i < cexDropEvent.dropCountCC.countItems; i++)
		{
			cexDropEvent.handlerCC(aIndex, MapNumber, x, y, type, level, dur , Op1, Op2, Op3, LootIndex, NewOption, SetOption);
		}
	}
	else {
		cexDropEvent.handlerCC(aIndex, MapNumber, x, y, type, level, dur , Op1, Op2, Op3, LootIndex, NewOption, SetOption);
...
{
	// Drop Count
	dropCountBC.countRate = Config.GetInt(0, 100, 0, "DropCount", "DropCountRate", RMSTEventDropBC);
	dropCountBC.countItems = Config.GetInt(1, 10, 1, "DropCount", "DropCountItems", RMSTEventDropBC);

	// Drop List

	FILE* fp;
	fp = fopen(RMSTEventDropBC, "r");
	rewind(fp);
	...
	// Log
	cLog.ConsoleOutPut(1, cLog.c_Cyan, cLog.t_Default, "[DropEvent System -> BC] Load sucsessfully.");
}
...
{
	// Drop Count
	dropCountIT.countRate = Config.GetInt(0, 100, 0, "DropCount", "DropCountRate", RMSTEventDropIT);
	dropCountIT.countItems = Config.GetInt(1, 10, 1, "DropCount", "DropCountItems", RMSTEventDropIT);

	// Drop List

	FILE* fp;
	fp = fopen(RMSTEventDropIT, "r");
	rewind(fp);
	...
	// Log
	cLog.ConsoleOutPut(1,cLog.c_Cyan, cLog.t_Default, "[DropEvent System -> IT] Load sucsessfully.");
}
...
{
	// Drop Count
	dropCountWW.countRate = Config.GetInt(0, 100, 0, "DropCount", "DropCountRate", RMSTEventDropWW);
	dropCountWW.countItems = Config.GetInt(1, 10, 1, "DropCount", "DropCountItems", RMSTEventDropWW);

	// Drop List

	FILE* fp;
	fp = fopen(RMSTEventDropWW, "r");
	rewind(fp);
	...
	// Log
	cLog.ConsoleOutPut(1, cLog.c_Cyan, cLog.t_Default, "[DropEvent System -> WW] Load sucsessfully.");
}
...
{
	// Drop Count
	dropCountCC.countRate = Config.GetInt(0, 100, 0, "DropCount", "DropCountRate", RMSTEventDropCC);
	dropCountCC.countItems = Config.GetInt(1, 10, 1, "DropCount", "DropCountItems", RMSTEventDropCC);

	// Drop List

	FILE* fp;
	fp = fopen(RMSTEventDropCC, "r");
	rewind(fp);
	...
	// Log
	cLog.ConsoleOutPut(1, cLog.c_Cyan, cLog.t_Default, "[DropEvent System -> CC] Load sucsessfully.");
}
