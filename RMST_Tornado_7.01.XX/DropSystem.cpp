...
void cDropSystem::LoadConfig()
{
	...
	if(fp == NULL)
	{
		cLog.ConsoleOutPut(1, cLog.c_Cyan, cLog.t_Default, "[Drop System] System not active. Config file not found: %s", RMSTDrop);
		return;
	}
	...
	cLog.ConsoleOutPut(1, cLog.c_Cyan, cLog.t_Default, "[Drop System] Load sucsessfully.");
}
...
void cDropSystem::CreateAndDrop(int aIndex, BYTE MapNumber, BYTE x, BYTE y, int LootIndex, s_DropSystemItem&i)
{
	...
	// Log
	cLog.ConsoleOutPut(1, cLog.c_Green, cLog.t_Default, "[Drop System] Drop Item -> Map: %d (%d;%d) Item: %d;%d", MapNumber, x, y, i.iGroup, i.iId);
}
