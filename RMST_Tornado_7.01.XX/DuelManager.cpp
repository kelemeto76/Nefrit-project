...
void DuelSystem::Run()
{
	for(short i = 0; i < MAX_DUEL_ROOMS; ++i)
	{
		if(this->g_DuelRooms[i].RoomFree == FALSE)
		{
			...
			for(short u = 0; u < MAX_DUEL_SEPARATORS; u++)
			{
				...
				// ----
				if(Util.gObjIsConnected(this->g_DuelRooms[i].szSeparators[u]->m_Index))
				{
					// ----
					if(this->g_DuelRooms[i].szSeparators[u]->MapNumber != 0x40)
					{
						...
						{
							cLog.ConsoleOutPut(1, cLog.c_Blue,cLog.t_Duel,"[Duel System] Spectator [%s] Leave [%d] Room", 
				...
				else
				{
					if(Config.Duel.Logging)
					{
						cLog.ConsoleOutPut(1, cLog.c_Blue,cLog.t_Duel,"[Duel System] Spectator Leave Room [%d]", i + 1);
					}
					// ----
					this->g_DuelRooms[i].szSeparators[u] = NULL;
					// ----
			...
			{
				if(Util.gObjIsConnected(this->g_DuelRooms[i].szlpObjOne->m_Index) == FALSE || 
				   Util.gObjIsConnected(this->g_DuelRooms[i].szlpObjTy->m_Index)  == FALSE )
				{
					this->RoomReset(i);
					continue;
				}
				...
				// ----
				else if(this->g_DuelRooms[i].szlpObjOne->Connected < 3 ||
						this->g_DuelRooms[i].szlpObjTy->Connected  < 3)
				{
					this->RoomReset(i);
					continue;
				}
			...
			if(this->g_DuelRooms[i].dwTicketCount != 0)
			{
				if(this->g_DuelRooms[i].dwTicketCount < GetTickCount())
				{
					...
					// ---
							if(Config.Duel.Logging)
		{
					cLog.ConsoleOutPut(1, cLog.c_Blue,cLog.t_Duel,"[Duel System] Room [%d] Cleaned", i + 1);
							}
				}
			}
			// ----
			   ...
			   this->g_DuelRooms[i].szWinner == FALSE)
			{
				for(short s = 0; s < MAX_DUEL_SEPARATORS; s++)
				{
					...
					{
						if(AddTab[this->g_DuelRooms[i].szSeparators[s]->m_Index - OBJECT_MIN].m_iSpecVisible == 1)
						{
							...
							// ----
									if(Config.Duel.Logging)
		{
							cLog.ConsoleOutPut(1, cLog.c_Blue,cLog.t_Duel,"[Duel System]Character [%s] Added Invisible", 
												    this->g_DuelRooms[i].szSeparators[s]->Name);
									}
							// ----
							AddTab[this->g_DuelRooms[i].szSeparators[s]->m_Index - OBJECT_MIN].m_iSpecVisible = 0;
						}
					}
				}
			}
	...
	for(short n = 0; n < MAX_DUEL_ROOMS; n++)
	{
		...
		{
			if(Util.gObjIsConnected(this->g_DuelRooms[n].szlpObjOne->m_Index) == FALSE || 
			   Util.gObjIsConnected(this->g_DuelRooms[n].szlpObjTy->m_Index)  == FALSE )
			{
				this->RoomReset(n);
				continue;
			}
			// ----
			else if(this->g_DuelRooms[n].szlpObjOne->Connected < 3 ||
					this->g_DuelRooms[n].szlpObjTy->Connected  < 3)
			{
				this->RoomReset(n);
				continue;
			}
...
{ 
	...
	// ----
			if(Config.Duel.Logging)
		{
	cLog.ConsoleOutPut(1, cLog.c_Blue,cLog.t_Duel,"[Duel System] Room [%d] Results [%s][%s] VS [%s][%s] Rank [%d] : [%d]", 
...
{ 
	...
	if(Util.gObjIsConnected(lpObj->m_Index) == FALSE)
	{ 
		return;
	}
	// ----
...
{ 
	...
	AddTab[aIndex - OBJECT_MIN].m_iDuelRoom = -1;
	lpObj->m_iDuelUser					 = -1;
...
void DuelSystem::RoomReset(short iRoom, bool dontMove, bool dontSendEnd)
{
	...
	{
		if(Util.gObjIsConnected(this->g_DuelRooms[iRoom].szlpObjOne->m_Index))
		{
	...
	{
		if(Util.gObjIsConnected(this->g_DuelRooms[iRoom].szlpObjTy->m_Index))
		{
			if(dontSendEnd == false)
			{
	...
	// ----
			if(Config.Duel.Logging)
		{
	cLog.ConsoleOutPut(1, cLog.c_Blue,cLog.t_Duel,"[Duel System] Room [%d] Has Restarted", iRoom +1);
			}
}
// ----------------------------------------------------------------------------------------------------------------
...
{ 
	...
	if(Points >= MAX_DUEL_WIN_POINTS)
	{
		...
		// ----
		cLog.ConsoleOutPut(1, cLog.c_Blue,cLog.t_Duel,"[Duel System] [%s][%s] Has Won Duel, Looser [%s][%s] Room [%d]",
			...
			if(this->g_DuelRooms[lpMsg->btRoomIndex].szSeparators[i] == NULL)
			{
				if(gObjMoveGate(lpObj->m_Index, g_DuelGates[lpMsg->btRoomIndex].SeparatorGate))
				{
					...
					// ----
							if(Config.Duel.Logging)
		{
					cLog.ConsoleOutPut(1, cLog.c_Blue,cLog.t_Duel,"[Duel System][%s][%s] Spectator join to room: [%d]", 
					...
					// ----
					AddTab[lpObj->m_Index - OBJECT_MIN].m_iSpecVisible = 1;
					// ----
					return;
				}
...
void DuelSystem::RecvDuelAnswer(LPOBJ lpObj, PMSG_DUEL_ANSWER_START* lpMsg)
{
	...
	int aIndex = lpObj->m_Index;
	// ----
	int iDuelRoom = AddTab[aIndex - OBJECT_MIN].m_iDuelRoom;
	// ----
	...
	{
		if(!Util.gObjIsConnected(iDuelIndex))
		{
			return;
		}
		// ----
		...
		if(gObjs->m_iDuelUserReserved == lpObj->m_Index)
		{
			...
			if(!strcmp(szDuelName, szDuelName2))
			{
				...
				else if (lpMsg->bDuelOK == true)
				{
					...
					// ----
					Chat.MessageAllLog(0, 0, cLog.c_Cyan, cLog.t_Duel, lpObj, "[Duel System] Duel Started [%s] VS [%s] on Room[%d]", lpObj->Name, gObjs->Name, iDuelRoom + 1);

					cLog.ConsoleOutPut(1, cLog.c_Blue,cLog.t_Duel,"[Duel System] Duel Started [%s][%s] VS [%s][%s] on Room[%d]",
...
void DuelSystem::RecvDuelRequest(LPOBJ lpObj, PMSG_DUEL_REQUEST_START* lpMsg)
{
	...
	if(!OBJECT_MAXRANGE(iDuelIndex))
	{
		cLog.ConsoleOutPut(1, cLog.c_Blue,cLog.t_Duel,"[Duel System] [%s][%s] Try duel with NO_PLAYER object!", lpObj->AccountID, lpObj->Name);
		return;
	}
	// ----
	...
	// ----
	if(Util.gObjIsConnected(iDuelIndex) == FALSE)
	{
		return;
	}
	// ----
	...
	int aIndex								 = lpObj->m_Index;
	// ----
	AddTab[aIndex - OBJECT_MIN].m_iDuelRoom	 = iDuelRoom;
	int uIndex								 = gObjs->m_Index;
	AddTab[uIndex - OBJECT_MIN].m_iDuelRoom	 = iDuelRoom;
	// ----
	...
	// ----
			if(Config.Duel.Logging)
		{
	cLog.ConsoleOutPut(1, cLog.c_Blue,cLog.t_Duel,"[Duel System][%s][%s] Requested to Start Duel to [%s][%s] on Room [%d]",
							lpObj->AccountID, lpObj->Name, gObjs->AccountID, gObjs->Name, iDuelRoom + 1);
			}
}
// ----------------------------------------------------------------------------------------------------------------
...
bool DuelSystem::IsDuelEnable(int aIndex)
{
	...
	// ----
	if(Util.gObjIsConnected(aIndex) == TRUE)
	{
...
void DuelSystem::SetDuelOption(int lpObj, BOOL bState)
{
	...
	// ----
	if(Util.gObjIsConnected(lpObj) == TRUE)
	{
		if(bState == 0)
		{
			gObj->m_Option = 0;
...
{
	if(Util.gObjIsConnected(lpObj->m_Index))
	{
		AddBuff(lpObj, 103, 20, 10, 0, 0, 3600);
		// ----
				if(Config.Duel.Logging)
		{
		cLog.ConsoleOutPut(1, cLog.c_Blue,cLog.t_Duel,"[Duel System] Winner : [%s][%s] received a Gladiator Glory Buff",
