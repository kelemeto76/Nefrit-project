...
//Описую структуру подробнее
typedef struct gQuest
{
	int TakingQuest; // Переменная состояния квеста ( взят или невзят )

	/*Массивы из 10 елементов для того чтобы можна было в одном квесте 
	было задания поиска до 10 видов шмоток либо до 10 видов мобов*/

	//Для квестов связаных с убийством мобов
	int NeedKillMobs[10]; // Сколько монстров надо убить
	int KillMobs[10]; // Сколько убито
	int NumberMobs[10]; // Номера мобов

	// Для квестов связаных с поиском шмоток
	int NeedQuestItem[10]; // Сколько шмоток надо найти
	int GetingQuestItems[10]; // Сколько нашли
	int NumberQuestItems[10]; // Номера шмоток ( ITEMGET(x,y) )
}gQuest;

typedef struct ADDITIONALSTRUCT 
{							
	sDate PostBanDate;	 
	...
	int IsVipServer;
	int VipMinutesServer;

	int IsVipInGame;
	int IsInGameVipMinutes;

	OBJECTSTRUCT* ArcherObj;

	//AntiFoul
	int AntiFoulWarnings;
	int AntiFoulBanMin;
	//Delay flags 
	bool MossBuyDelay;
	//Quest System
	gQuest QSystem; // Прочитай описания структуры выше
...
//extern OBJECTSTRUCT *ObjTable;
typedef OBJECTSTRUCT * LPOBJ;
//extern OBJECTSTRUCT *ObjTab;
extern ADDITIONALSTRUCT AddTab[OBJECT_MAX];	 
